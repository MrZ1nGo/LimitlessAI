...
app.post('/api/login', async (req, res) => {
  try {
    ...
    const { data: user } = await supabase
      .from('users').select('*')
      .or(`email.eq.${input},username.ilike.${input}`)
      .maybeSingle();
...
app.post('/api/images', adminMiddleware, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from('images').insert({ character_id, url }).select().single();
    if (error) return res.status(500).json({ error: error.message });
    res.json(data);
  } catch (err) {
    res.status(500).json({ error: err.message });
    ...
    const { data, error } = await supabase
      ...
      .select(`
        *,
        ...
        comment_likes ( user_id, is_author_like )
      `)
...
// ===== КОММЕНТАРИИ — закрепить/открепить =====
app.patch('/api/comments/:id/pin', adminMiddleware, async (req, res) => {
  try {
    const { pinned } = req.body;
    const { data, error } = await supabase
      .from('comments').update({ is_pinned: pinned })
...
app.post('/api/comments/:id/like', authMiddleware, async (req, res) => {
  try {
    ...
    if (isAuthorLike && !req.user.isAdmin)
      return res.status(403).json({ error: 'Only admin can use author like' });

    const { data: existing } = await supabase
      .from('comment_likes').select('id')
      .eq('comment_id', commentId)
      .eq('user_id', userId)
      .eq('is_author_like', isAuthorLike)
      .maybeSingle();
    ...
    if (existing) {
      ...
      await supabase.from('comment_likes')
        .insert({ comment_id: commentId, user_id: userId, is_author_like: isAuthorLike });
      res.json({ liked: true });
    }
  } catch (err) {
    res.status(500).json({ error: err.message });
